...
function ajouteStation(map,lat,lng,id,nom,adresse)
{
	if($("#station_depart_popover").attr("aria-describedby")!=undefined)
	{
	    ...
	    if(id == -1)
	    	StationDepart = Positionnerstationbus(map,lat,lng,id,nom,adresse);
	    else
	    	StationDepart=1;
	...
	} else if($("#station_arrivee_popover").attr("aria-describedby")!=undefined)
	{
	    ...
	    if(id == -1)
	   		 StationArrivee = Positionnerstationbus(map,lat,lng,id,nom,adresse);
	    else
	       StationDepart=1;
  ...
  else  if($("#station_modifie_popover").attr("aria-describedby")!=undefined || $(".bouton_a[form-id='modifie_data_bus']").attr("click")=="true")
  {
  ...
  }else  if($("#station_supprime_popover").attr("aria-describedby")!=undefined || $(".bouton_a[form-id='supprime_data_bus']").attr("click")=="true")
  {
        ...
        $(".popover label[data-id='supprimer_bus_station_nom']").html("Station Nom :"+nom);
  }
}
...
function add(event) {
    ...
    if($("#trajet_popover").attr("aria-describedby")!=undefined)
    {
      ...
      $("#form_ajout_data_bus").html($("#form_ajout_data_bus").html()+html);
...
$(document).ready(function(){
    ...
    $(".bouton_a").click(function(){
        ...
        clear_trajet();
       if(form_id=="ajout_data_bus")
       {
           trace_trajet_bus(-1);
       }
       else if(form_id=="modifie_data_bus")
       {
         var valeur=$("#modifier_bus_id_bus").val();
         if(valeur!=-1)
          trace_trajet_bus(valeur);
       }
       else if(form_id=="supprime_data_bus")
       {
          var valeur=$("#supprimer_bus_id_bus").val();
          if(valeur!=-1)
           trace_trajet_bus(valeur);
       }
       $(".form_bus").hide();
...
function trace_trajet_bus(id) {
    $(".wait").show();
  $.ajax({
    type : "POST",
    contentType : "application/json",
     url: getContextPath()+"/getStationBus/"+id,
    data : JSON.stringify({}),
    dataType : 'json',
    timeout : 100000,
    success : function(data) {
...
function click_trajet_modifier()
{
  ...
  if($("#trajet_modifie_popover").attr("aria-describedby")!=undefined || $(".bouton_a[form-id='modifie_data_bus']").attr("click")=="true")
  {
    ...
    $.ajax({
           type: "POST",
           url: getContextPath()+"/getBusByStationDepartAndArrivee/"+this.id_depart+"/"+this.id_arrivee,
           data: {},
  ...
  }else if($("#trajet_supprime_popover").attr("aria-describedby")!=undefined || $(".bouton_a[form-id='supprime_data_bus']").attr("click")=="true"){
    if($("#trajet_supprime_popover").attr("aria-describedby")==undefined)
      $("#trajet_supprime_popover").popover("show");
    $(".popover label[data-id='supprimer_bus_trajet_id_label']").html("Trajet :"+this.id);
    ...
    /*$.ajax({
           ...
           data: {},
           success: function(response){
               $(".popover select[data-id='supprimer_bus_trajet_tamway']").multipleSelect("setSelects", response.split(","));
           },
           error: function(e){
              console.log("ERROR: ", e);
             }
...
function ajoute_bus() {
	 $.ajax({
	        type: "POST",
	        url: getContextPath()+"/addBus",
	        data: $("#form_ajoute_data_bus_bus").serialize(),
	        success: function(response){
	         if(response!="")
	        	 {
                $(".popover").popover('hide');
                $("select[data-id='modifier_trajet_bus']").html($("select[data-id='modifier_trajet_bus']").html()+response);
                $("select[data-id='supprimer_bus_bus']").html($("select[data-id='supprimer_bus_bus']").html()+response);
                $("select[data-id='trajet_bus']").html($("select[data-id='trajet_bus']").html()+response);
	        	 }
	        },
	        error: function(e){
    	       console.log("ERROR: ", e);
...
function ajoute_bus_trajet() {
	 $.ajax({
	        type: "POST",
	        url: getContextPath()+"/addbusData",
	        data: $("#form_ajout_data_bus").serialize(),
	        success: function(response){
	         if(response!="")
	        	 {
...
function select_bus_modifier(elem) {
     $(".popover").popover('hide');
  clear_trajet();
  var val=elem.value;
  $("#modifier_bus_id").val(val);
  if(val!=-1)
  {
     trace_trajet_bus(val);

     $.ajax({
       type : "POST",
       contentType : "application/json",
        url: "http://localhost:8080/GuidezMoi/getBus/"+val,
       data : JSON.stringify({}),
       dataType : 'json',
       timeout : 100000,
       success : function(data) {

         if($("#bus_modifie_popover").attr("data-exist")=="false")
         {
           $("input[data-id='modifier_bus_numero']").val(data.NumDeLigne);
           $("input[data-id='modifier_bus_prix']").val(data.Prix);
         }
         else {
           $("#modifier_bus_numero").val(data.NumDeLigne);
           $("#modifier_bus_prix").val(data.Prix);
         }
       },
       error : function(e) {
         console.log("ERROR: ", e);
...
function select_bus_supprimer(elem) {
     $(".popover").popover('hide');
  clear_trajet();
  var val=elem.value;
    $("#supprimer_station_bus_id").val(val);
    $("#supprimer_trajet_bus_id").val(val);
    $("#supprimer_bus_id").val(val);
